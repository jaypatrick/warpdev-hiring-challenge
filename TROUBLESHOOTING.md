...
- Use the correct path: `awk -f src/mars_mission_analyzer.awk data/space_missions.log`
...
3. **Very large file**: The full dataset takes time to process (~10MB file with 100k+ lines)
   ...
   - Use verbose mode to see progress: `awk -v verbose=1 -f src/mars_mission_analyzer.awk data/space_missions.log`
...
1. **Wrong file**: Make sure you're using the correct log file
   ```bash
   ls -lh data/space_missions.log  # Should be ~10MB
   ```
...
1. **Stderr mixed with stdout**: Redirect stderr to separate output
   ```bash
   awk -v format=json -f src/mars_mission_analyzer.awk data/space_missions.log 2>/dev/null
   ```

2. **Partial output**: Make sure the command completes successfully
   ```bash
   awk -v format=json -f src/mars_mission_analyzer.awk data/space_missions.log > output.json
...
1. **Disable verbose mode** for production use:
   ```bash
   awk -f src/mars_mission_analyzer.awk data/space_missions.log  # Fast
   awk -v verbose=1 -f src/mars_mission_analyzer.awk data/space_missions.log  # Slower
   ```

2. **Limit top-N results**:
   ```bash
   awk -v top=10 -f src/mars_mission_analyzer.awk data/space_missions.log
   ```
...
```bash
file -I data/space_missions.log  # Should show charset=utf-8

# If needed, convert:
iconv -f ISO-8859-1 -t UTF-8 data/space_missions.log > space_missions_utf8.log
```
...
### Enable Verbose Mode
```bash
awk -v verbose=1 -f src/mars_mission_analyzer.awk data/space_missions.log 2>&1 | less
```
...
### View Full Record
```bash
awk -v verbose=1 -f src/mars_mission_analyzer.awk data/space_missions.log | grep "Full Record"
```
...
# CSV output
awk -v format=csv -v top=10 -f src/mars_mission_analyzer.awk data/space_missions.log | column -t -s,
```
